...
@Component({
  selector: 'esp-grid-cell-price-input',
  templateUrl: './grid-cell-price-input.component.html',
  styleUrls: ['./grid-cell-price-input.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => GridCellPriceInputComponent),
      multi: true,
    },
  ],
  standalone: true,
...
export class GridCellPriceInputComponent extends GridCellInputDirective {
  ...
  beforeOnChange(source: Observable<string>): Observable<number | null> {
    return source.pipe(toNumberOrNull((value) => parseFloat(value)));
  }
}
